...
function addSubmit() {
    if (validationInput(bookmarkName)) {
        var existingBookmark = bookMark.find(mark => mark.name.toLowerCase() === bookmarkName.value.toLowerCase());
        if (existingBookmark) {
    ...
    for (var i = 0; i< bookMark.length; i++) {
        carton +=`
                        <tr>
                            ...
                            <td>
                         <button onclick="openURL('${list[i].url}')" class="btn btn-visit">
...
function validationInput(element) {
    var regex ={
        ...
        bookmarkURL:/^https?:\/\/(www\.)?[-a-zA-Z0-9@:%.\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()!@:%\+.~#?&\/\/=]*)$/gm,
